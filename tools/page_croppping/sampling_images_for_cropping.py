...
def get_all_s3_keys_from_bucket():
    ...
    obj_keys = []
    final_dict = {}
    base_directories = get_base_directories(s3_client=s3_client, bucket_name=PAGE_CROPPPING_BUCKET)
    for base_dir in base_directories:
        prefix = (base_dir.get('Prefix'))[:-1]
        obj_keys = list_obj_keys(prefix, s3_client=s3_client, bucket_name=PAGE_CROPPPING_BUCKET)
        curr[prefix] = {
            "s3_keys": obj_keys
        }
